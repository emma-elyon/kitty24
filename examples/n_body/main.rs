...
fn main() -> Result<(), std::io::Error> {
	let source = std::fs::read_to_string("examples/n_body.kitty.script").unwrap();
	...
	std::fs::write("n_body.kitty.asm", &assembly).unwrap();

	let assembly = std::fs::read_to_string("n_body.kitty.asm").unwrap();
