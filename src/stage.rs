...
pub struct Stage<F> where F: FnMut([u32; 16]) {
    pipeline: Pipeline,
    bindings: Bindings,
...
impl<F> Stage<F> where F: FnMut([u32; 16]) {
    pub fn new(context: &mut Context, virtual_machine: VirtualMachine<F>) -> Stage<F> {
		...
		let audio: Arc<Mutex<VecDeque<f32>>> = Arc::new(Mutex::new(VecDeque::new()));
		...
		let device = host.default_output_device().unwrap(); // TODO: Run without audio.
		let config = device.default_output_config().unwrap();
		let channels = config.channels() as usize;
		let err_fn = |err| eprintln!("an error occurred on stream: {}", err);
		let stream_buffer = audio.clone();
		let stream = device.build_output_stream(
			&config.into(),
			move |data: &mut [f32], _: &cpal::OutputCallbackInfo| {
				let mut buffer = stream_buffer.lock().unwrap();
				'outer: for frame in data.chunks_mut(channels) {
					if let Some(value) = buffer.pop_front() {
						...
						eprintln!("Audio buffer underflow");
						break 'outer;
					}
				}
			},
...
impl<F> EventHandler for Stage<F> where F: FnMut([u32; 16]) {
    fn update(&mut self, context: &mut Context) {
		let now = date::now();
		if self.reboot {
			self.reboot = false;
			self.virtual_machine.reboot();
		}
		self.virtual_machine.set_keys(&self.keys);
		let text_input = self.text_input.drain(..).collect();
		self.virtual_machine.set_text(text_input);
		self.busy_total += self.virtual_machine.run(&mut self.display, Some(&self.audio));
	...
	fn char_event(
		...
		if character.is_ascii() && character as u8 >= b' ' {
			self.text_input.push_back(character as u8);
		}
	}
	...
	fn key_down_event(
		...
		if keycode == KeyCode::R && keymods.ctrl && !keymods.alt && !keymods.shift {
			self.reboot = true;
		} else {
...
/// Convert KeyCode to canonical kitty24 code.
fn key_code_to_u16(key_code: KeyCode) -> u16 {
	match key_code {
		...
		KeyCode::Right => 0x102,
		KeyCode::Down => 0x103,
		KeyCode::LeftControl => 0x104,

		// Unordered, not configured.
		...
		KeyCode::KpAdd => 0x22,
		KeyCode::KpEnter => 0x10, // TODO: This clashes with ::Enter
