...
impl Ram {
	...
	pub fn block_transfer(&mut self) -> usize {
        let src_modulo =
			((self.memory[memory::BLOCK_SRC_MOD + 0] as u32) << 8) |
			((self.memory[memory::BLOCK_SRC_MOD + 1] as u32) << 0);
        let src_width =
			((self.memory[memory::BLOCK_SRC_WIDTH + 0] as u32) << 8) |
			((self.memory[memory::BLOCK_SRC_WIDTH + 1] as u32) << 0);
        let src_height =
			((self.memory[memory::BLOCK_SRC_HEIGHT + 0] as u32) << 8) |
			((self.memory[memory::BLOCK_SRC_HEIGHT + 1] as u32) << 0);
        let src_address =
			((self.memory[memory::BLOCK_SRC_ADDRESS + 0] as u32) << 16) |
			((self.memory[memory::BLOCK_SRC_ADDRESS + 1] as u32) <<  8) |
			((self.memory[memory::BLOCK_SRC_ADDRESS + 2] as u32) <<  0);
        let dst_modulo =
			((self.memory[memory::BLOCK_DST_MOD + 0] as u32) << 8) |
			((self.memory[memory::BLOCK_DST_MOD + 1] as u32) << 0);
        let dst_width =
			((self.memory[memory::BLOCK_DST_WIDTH + 0] as u32) << 8) |
			((self.memory[memory::BLOCK_DST_WIDTH + 1] as u32) << 0);
        let dst_height =
			((self.memory[memory::BLOCK_DST_HEIGHT + 0] as u32) << 8) |
			((self.memory[memory::BLOCK_DST_HEIGHT + 1] as u32) << 0);
        let dst_address =
			((self.memory[memory::BLOCK_DST_ADDRESS + 0] as u32) << 16) |
			...
			for src_offset in 0..src_width * src_height {
				if dst_modulo == 0 {
					...
					if blit_0 || blit_2 || pixel != 0 {
						if blit_2 {
							let byte =
								((self[memory::BLOCK_REMAP as u32 + ((pixel as u32 >> 0) & 0x03)] & 0x03) << 0) |
								((self[memory::BLOCK_REMAP as u32 + ((pixel as u32 >> 2) & 0x03)] & 0x03) << 2) |
								((self[memory::BLOCK_REMAP as u32 + ((pixel as u32 >> 4) & 0x03)] & 0x03) << 4) |
								((self[memory::BLOCK_REMAP as u32 + ((pixel as u32 >> 6) & 0x03)] & 0x03) << 6);
							self[dst_address + dst_offset] = byte
						} else {
							self[dst_address + dst_offset] = self[memory::BLOCK_REMAP as u32 + pixel as u32];
						}
					}
				} else {
