...
impl TryFrom<Pair<'_, Rule>> for Statement {
    ...
    fn try_from(pair: Pair<'_, Rule>) -> Result<Self, Self::Error> {
        ...
        let statement = match inner_pair.as_rule() {
			...
			Rule::Return => Statement::Return(pairs.next().map(|pair| pair.into_inner().try_into().unwrap()).unwrap_or(Expressions(Vec::new()))),
            Rule::Break => Statement::Break(pairs.next().map(|pair| pair.as_str().to_string())),
            Rule::Continue => Statement::Continue(pairs.next().map(|pair| pair.as_str().to_string())),
...
fn parse_primary_expression(pair: Pair<Rule>) -> Result<Expression, std::io::Error> {
    let expression = match pair.as_rule() {
        ...
        Rule::ConditionalMultiple => { // else if
            let mut pairs = pair.into_inner();
            let predicate = pairs.next().unwrap().try_into()?;
            let positive_block = pairs.next().unwrap();
            let positive_block = positive_block.try_into()?;
			...
			Expression::ConditionalMultiple(Box::new(predicate), positive_block, nested_conditionals, negative_block)
        }
        Rule::Call => {
            let mut pairs = pair.into_inner();
        ...
        Rule::StringLiteral => {
			...
			for pair in pairs {
				string = string + match pair.as_rule() {
		...
		Rule::CharacterLiteral => {
			...
			let b = pairs.next().map(|pair| parse_character_literal(pair).unwrap() as u32);
			let c = pairs.next().map(|pair| parse_character_literal(pair).unwrap() as u32);
			let a = a << if b.is_none() { 0 } else if c.is_none() { 8 } else { 16 };
			let b = if b.is_none() { 0 } else if c.is_none() { b.unwrap() } else { b.unwrap() << 8 };
