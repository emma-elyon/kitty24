...
fn parse_statement(statement: Pair<Rule>)
-> Result<Statement, std::io::Error> {
	match statement.as_rule() {
		// let r, 42
		Rule::Instruction =>
			Ok(Statement::Instruction(parse_instruction(statement)?)),
		
		// label:
		Rule::LabelDefinition =>
			Ok(Statement::Label(parse_label(statement))),
...
fn parse_instruction(instruction: Pair<Rule>)
-> Result<Instruction, std::io::Error> {
	let instruction = instruction.into_inner().next().unwrap();
	match instruction.as_rule() {
		Rule::Or =>
			Ok(Instruction::Or(r(instruction, 0)?)),
		Rule::Nor =>
			Ok(Instruction::Nor(r(instruction, 0)?)),
		Rule::And =>
			// Default was 0x0000FF, is now 0; virtual machine only ignores 0.
			Ok(Instruction::And(r(instruction, 0)?)),
		Rule::Xor =>
			Ok(Instruction::Xor(r(instruction, 0)?)),
		Rule::Add =>
			Ok(Instruction::Add(r(instruction, 0)?)),
		Rule::Sub =>
			Ok(Instruction::Sub(r(instruction, 0)?)),
		Rule::Mul =>
			Ok(Instruction::Mul(r(instruction, 1)?)),
		Rule::ShiftLeft =>
			Ok(Instruction::ShiftLeft(rs(instruction)?)),
		Rule::ShiftRight =>
			Ok(Instruction::ShiftRight(rs(instruction)?)),
		Rule::RotateLeft =>
			Ok(Instruction::RotateLeft(rs(instruction)?)),
		Rule::RotateRight =>
			Ok(Instruction::RotateRight(rs(instruction)?)),
		Rule::ArithmeticShiftRight =>
			Ok(Instruction::ArithmeticShiftRight(rs(instruction)?)),
		Rule::Less =>
			Ok(Instruction::Less(r(instruction, 0)?)),
		Rule::SignedLess =>
			Ok(Instruction::SignedLess(r(instruction, 0)?)),
		Rule::If =>
			Ok(Instruction::If(if_(instruction)?)),
		Rule::Let =>
			Ok(Instruction::Let(i(instruction)?)),
		Rule::Load =>
			Ok(Instruction::Load(m(instruction)?)),
		Rule::Store =>
			Ok(Instruction::Store(m(instruction)?)),
		Rule::Pop =>
			Ok(Instruction::Pop(m(instruction)?)),
		Rule::Push =>
			Ok(Instruction::Push(m(instruction)?)),
		Rule::Data =>
			Ok(Instruction::Data(data(instruction)?)),
		_ => unreachable!("{:?}", instruction)
	}
}

/// Parse global or local label. Local label names are stored as-is in the AST.
fn parse_label(label: Pair<Rule>)
-> Label {
...
/// 
/// `Register ~ "," ~ Value16`
fn i(instruction: Pair<Rule>)
-> Result<I, std::io::Error> {
...
-> Result<M, std::io::Error> {
	...
	if let Some(argument) = arguments.next() {
		match argument.as_rule() {
			Rule::Register =>
				source = Some(argument.try_into()?),

			Rule::SignedDecimal10 =>
	...
	Ok(M { destination, source, immediate, byte_count })
}

/// Parse R-instruction, register-register (`add r1, r4, r9, 2`)
/// 
/// `Register ~ ("," ~ Register){0, 1} ~ ("," ~ Value8)?`
fn r(instruction: Pair<Rule>, default: u8)
-> Result<R, std::io::Error> {
	...
	Ok(R { destination, source, target, immediate })
}

/// Parse RS-instruction, register-register shift (`shl r1, r1, r0, 8`)
fn rs(instruction: Pair<Rule>)
-> Result<RS, std::io::Error> {
	let mut arguments = instruction.clone().into_inner();
	...
	Ok(RS { destination, source, target, immediate })
}
...
fn parse_16(number: Pair<Rule>) -> Result<u16, std::io::Error> {
	...
	match number.as_rule() {
		Rule::Hexadecimal16 =>
			u16::from_str_radix(number.as_str(), 0x10).map_err(parse_error),

		Rule::Binary16 =>
			u16::from_str_radix(number.as_str(), 0b10).map_err(parse_error),

		Rule::Decimal16 =>
			number.as_str().parse::<u16>().map_err(parse_error),

		Rule::SignedDecimal16 =>
			number.as_str().parse::<i16>()
...
fn parse_8(number: Pair<Rule>) -> Result<u8, std::io::Error> {
	...
	match number.as_rule() {
		Rule::Hexadecimal8 =>
			u8::from_str_radix(number.as_str(), 0x10).map_err(parse_error),

		Rule::Binary8 =>
			u8::from_str_radix(number.as_str(), 0b10).map_err(parse_error),

		Rule::Decimal8 =>
			number.as_str().parse::<u8>().map_err(parse_error),
...
fn parse_5(number: Pair<Rule>) -> Result<u8, std::io::Error> {
	...
	match pair.as_rule() {
		Rule::Hexadecimal5 =>
			u8::from_str_radix(pair.as_str(), 0x10).map_err(parse_error),

		Rule::Binary5 =>
			u8::from_str_radix(pair.as_str(), 0b10).map_err(parse_error),

		Rule::Decimal5 =>
			pair.as_str().parse::<u8>().map_err(parse_error),
